...
RegisterNetEvent("wk-fotovogn:sendvogn")
AddEventHandler("wk-fotovogn:sendvogn", function(list)
    fotovogne=list
end)

RegisterNetEvent("wk-fotovogn:createdefault")
AddEventHandler("wk-fotovogn:createdefault", function(list,current)
...
RegisterNetEvent("wk-fotovogn:sendalert")
AddEventHandler("wk-fotovogn:sendalert", function(veh,tax,speed)
...
function checkSpeed(limit,net)
    ...
    if check ~= 0 then
        ...
        if kmhspeed > limit+4 and driver == pP then
            ...
            TriggerServerEvent('wk-fotovogn:betal',kmhspeed,limit,((net) and VehToNet(vehicle) or false))
        end
        Wait(3000)
    end
end
        ...
        if inVeh ~= 0 and GetEntityModel(inVeh) == vhash then
            if fotovogne[VehToNet(inVeh)] ~= nil then
                if GetEntitySpeed(inVeh) > 1 then
                    TriggerServerEvent("wk-fotovogn:removevogn",VehToNet(inVeh))
                end
            end
            if GetEntitySpeed(inVeh) < 1 then
                if isEditing then
                    ...
                    if IsControlJustPressed(0,176) then
                        if fotovogne[VehToNet(inVeh)] == nil then
                            if Vdist(coords["x"],coords["y"],coords["z"], xoffset,yoffset,coords["z"]) < 30.0 then
                                ...
                                TriggerServerEvent("wk-fotovogn:sendvogn",VehToNet(inVeh),{coords={xoffset,yoffset,coords["z"]},turned=true,limit=fart,radius=radius})
                            else
